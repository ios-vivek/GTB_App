...
public static func getAttributedString(str1: String, str2: String) -> NSMutableAttributedString{
    ...
    public static func getAddressFromLatlong(text: String) {
        WebServices.googleAddressFromLatLong(searchtext: text, forModelType: GoogleAddressLatLongResponse.self) { success in
...
//                        print(placemark.postalCode!)
    ...
    public static func getAddressFromZipcode(zip: String, successHandler: @escaping (_ successResult: LocationAddress) -> Void, errorHandler: @escaping (_ successResult: String) -> Void) {
        let geocoder = CLGeocoder()
        let params: [String: Any] = [CNPostalAddressPostalCodeKey: zip, CNPostalAddressISOCountryCodeKey: "US"]
                ...
                if placemark.count>0{
                   ...
                   // print(placemark.postalCode!)
                    if placemark.postalCode == nil{
                        errorHandler("Please enter zip code")
                    }else{
                    let address = LocationAddress()
                    if placemark.postalCode == nil{
                        address.city = ""
    ...
    public static func setGlobalAddress(addressWithLatLong: [ResultLatLong]){
        if !addressWithLatLong.isEmpty {
            ...
            let latLong : CLLocationCoordinate2D = CLLocationCoordinate2DMake(cllocation.coordinate.latitude, cllocation.coordinate.latitude)
