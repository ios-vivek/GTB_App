...
//let arr = ["Half", "Medium", "Full"]
    ...
    override func viewDidLoad() {
        ...
        */
        ...
        itemQty = Int(itemData.minqty) ?? 1
    
        if Cart.shared.getAllSizes(menu: restmenu, item: itemData).count == 1 {
            selectedSize = 0
            Cart.shared.itemSizes = [Sizes]()
            Cart.shared.itemSizes.append(Cart.shared.getAllSizes(menu: restmenu, item: itemData)[selectedSize])
        } else {
            Cart.shared.itemSizes = [Sizes]()
        }
    ...
    func getToppingFromApi() {
        let parameters: [String: AnyObject] = [
            ...
            //"restaurant_id" : "230623185919168" as AnyObject,//RestaurantCartDeatils.shared.restuarantID as AnyObject
            "item_id": "\(itemData.id)" as AnyObject

        ]
        UtilsClass.showProgressHud(view: self.view)
        WebServices.loadDataFromService(parameter: parameters, servicename: OldServiceType.options, forModelType: [RestToppingsResponse].self) { success in
            UtilsClass.hideProgressHud(view: self.view)
           // if let toppingList = success.data?. {
    ...
    @IBAction func minusAction() {
        if Int(itemData.minqty) ?? 1 < itemQty {
            itemQty = itemQty - 1
        }
        self.setAmountValue()
    }
    @IBAction func addItemInCart() {
      ...
      // let msg = RestaurantCartDeatils.shared.addAndRemoveToppins(selectedTopping: selectedOption, maxCount: Int(topping[indexPath.section - 1].ch!))
//        if msg.count > 0 {
//            self.showToast(message: msg, font: .systemFont(ofSize: 14.0))
//        } else {
//            
//            RestaurantCartDeatils.shared.itemData = itemData
//            RestaurantCartDeatils.shared.addInCart()
//            self.dismiss(animated: true) {
//                self.delegate?.itemAddedInTheCart()
//            }
//        }
    ...
    func setAmountValue() {
       ...
       // print("---amount\(Cart.shared.itemExtra)")


//                if instructionTxtView.text.uppercased().contains("extra".uppercased()) {
//                    Cart.shared.itemExtra = 0.5
...
//            }
        ...
        if selectedSize >= 0 {
            var newSize = Cart.shared.getAllSizes(menu: restmenu, item: itemData)[selectedSize]
...
extension ItemSizeSelectionPopupVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == 0 {
            return Cart.shared.getAllSizes(menu: restmenu, item: itemData).count
        }
        if selectedSize >= 0 {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            cell.sizeNameLbl.text = Cart.shared.getAllSizes(menu: restmenu, item: itemData)[indexPath.row].name
            cell.priceLbl.text = "\(UtilsClass.getCurrencySymbol())\(Cart.shared.getAllSizes(menu: restmenu, item: itemData)[indexPath.row].price)"
            cell.updateUIForSelectSize(indexPath: indexPath, sizes: Cart.shared.getAllSizes(menu: restmenu, item: itemData), selectedSize: selectedSize)
        }
        else if indexPath.section > 0 {
            if selectedSize >= 0 {
                
            }
            cell.updateUIForSelectOption(indexPath: indexPath, option: self.topping[indexPath.section - 1].optionlist!, isChecked: Cart.shared.isAddedOption(toppingHeading: self.topping[indexPath.section - 1].heading, optionHeading: self.topping[indexPath.section - 1].optionlist![indexPath.row].heading), sizes: selectedSize >= 0 ? Cart.shared.getAllSizes(menu: restmenu, item: itemData)[selectedSize] : nil)
         }
        else {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 0{
           ...
           // selectedOption = indexPath.row
            ...
            let option = SelectedOption(optionHeading: topping[indexPath.section - 1].optionlist![indexPath.row].heading, price: Cart.shared.getOptionsPrice(option: topping[indexPath.section - 1].optionlist![indexPath.row], sizes: selectedSize >= 0 ? Cart.shared.itemSizes[0] : nil))
            let toppingHeading = topping[indexPath.section - 1].heading
            let selectedOption = SelectedTopping(toppingHeading: toppingHeading, option: [option])
