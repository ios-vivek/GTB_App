...
//var numberOfPeople = 1
    ...
    func refreshView() {
        ...
        nameLbl.attributedText = getAttributedString(fstring: "Name:  ", sstring: "\(APPDELEGATE.userResponse?.customer.fname ?? "") \(APPDELEGATE.userResponse?.customer.lname ?? "")")
...
//       
    ...
    func bookDineFromApi() {
        ...
        let parameters: [String: AnyObject] = [
            ...
            "restaurant_id" : Cart.shared.tempRestDetails.id as AnyObject,
            "bdate" : "\(datesList[selectedDate].currectDateInFormate) \(timeSlots[selectedHour].time)" as AnyObject,
            "name": "\(APPDELEGATE.userResponse?.customer.fname ?? "") \(APPDELEGATE.userResponse?.customer.lname ?? "")" as AnyObject,
            "phone": "\(APPDELEGATE.userResponse?.customer.phone ?? "")" as AnyObject,
            "email": "\(APPDELEGATE.userResponse?.customer.email ?? "")" as AnyObject,
            "details": "\(txtView.text!)" as AnyObject,
            "peoples": "\(str)" as AnyObject
        ]
        print(parameters.json)
        
        UtilsClass.showProgressHud(view: self.view)
        WebServices.loadDataFromService(parameter: parameters, servicename: OldServiceType.bookdine, forModelType: DineBookedResponse.self) { success in
